...
class SynthIDLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.config.watermark_mode == "non-distortionary":
            ...
            is_repeated = torch.zeros((batch_size, 1), dtype=torch.bool, device=self.device)
...
class SynthID(BaseWatermark):
    ...
    def generate_watermarked_text(self, prompt: str, *args, **kwargs) -> str:
        ...
        self.logits_processor.state = None
    ...
    def batch_generate_watermarked_text(self, prompts: list[str], *args, **kwargs) -> str:
        ...
        encoded_watermarked_text = generate_with_watermark(**encoded_prompt)
        # truncate prompt
        encoded_watermarked_text = encoded_watermarked_text[:, encoded_prompt["input_ids"].shape[1]:]
        # Decode
    ...
    def detect_watermark(self, text: str, return_dict: bool = True, *args, **kwargs):
        ...
        score, p_value = self.detector.detect(g_values_np, mask_np)[0]  # Take first element as we have batch_size=1
        score = score[0]
        p_value = p_value[0]
        ...
        if return_dict:
            return {"is_watermarked": bool(is_watermarked), "score": float(score), "p_value": p_value}
        else:
            return (is_watermarked, float(score), p_value)

    def detect_watermark_tokens(self, inputs: torch.LongTensor, return_dict: bool = True, *args, **kwargs):
        """Detect watermark in the input tokens.
        
        Args:
            input (torch.LongTensor): Input tokens to detect watermark in
            return_dict (bool): Whether to return results as dictionary
        ...
        # Compute g-values for the input tokens
        g_values = self.logits_processor.compute_g_values(inputs)
        ...
        g_values_np = g_values.cpu().numpy()
        mask_np = combined_mask.cpu().numpy()
        score, p_value = self.detector.detect(g_values_np, mask_np)
        score = score[0]
    ...
    def detect_watermark_tokens_no_p_value(self, inputs: torch.LongTensor, return_dict: bool = True, *args, **kwargs):
        """Detect watermark in the input tokens.
        ...
        g_values_np = g_values.cpu().numpy()
        mask_np = combined_mask.cpu().numpy()
        score = self.detector.detect_no_p_value(g_values_np, mask_np)
