...
def display_result_single(args):
    ...
    # 根据question_id检索question.jsonl中对应的类别，按类别算平均并输出
    ...
    # Merge dataframes to get categories
    df = df.merge(questions, on="question_id", how="left")

    print("\n########## First turn ##########")
    df_1 = df[df["turn"] == 1].groupby("model")["score"].mean()
    print(df_1.sort_values(ascending=False))

    print("\n########## First turn by category ##########")
    df_cat = df[df["turn"] == 1].groupby(["model", "category"])["score"].agg(["mean", "count"])
    print(df_cat.sort_index())
    
    if args.bench_name == "mt_bench":
        print("\n########## Second turn ##########")
        df_2 = df[df["turn"] == 2].groupby("model")["score"].mean()
        print(df_2.sort_values(ascending=False))

        print("\n########## Second turn by category ##########")
        df_cat = df[df["turn"] == 2].groupby(["model", "category"])["score"].agg(["mean", "count"])
        print(df_cat.sort_index())

        print("\n########## Average ##########")
        df_3 = df[["model", "score"]].groupby(["model"])["score"].mean()
        print(df_3.sort_values(ascending=False))

        print("\n########## Average by category ##########")
        df_cat = df.groupby(["model", "category"])["score"].agg(["mean", "count"])
        print(df_cat.sort_index())
